...
@OnlyIn(Dist.CLIENT)
public class WallChange1GuiWindow extends ContainerScreen<WallChange1Gui.GuiContainerMod> {
	private World world;
	private int x, y, z;
	private PlayerEntity entity;
	private final static HashMap guistate = WallChange1Gui.guistate;
	public WallChange1GuiWindow(WallChange1Gui.GuiContainerMod container, PlayerInventory inventory, ITextComponent text) {
	...
	public void init(Minecraft minecraft, int width, int height) {
		...
		this.addButton(new Button(this.guiLeft + 15, this.guiTop + 8, 30, 20, new StringTextComponent("A"), e -> {
			if (true) {
				FnafModMod.PACKET_HANDLER.sendToServer(new WallChange1Gui.ButtonPressedMessage(0, x, y, z));
				WallChange1Gui.handleButtonAction(entity, 0, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 51, this.guiTop + 8, 30, 20, new StringTextComponent("B"), e -> {
			if (true) {
				FnafModMod.PACKET_HANDLER.sendToServer(new WallChange1Gui.ButtonPressedMessage(1, x, y, z));
				WallChange1Gui.handleButtonAction(entity, 1, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 96, this.guiTop + 8, 30, 20, new StringTextComponent("C"), e -> {
			if (true) {
				FnafModMod.PACKET_HANDLER.sendToServer(new WallChange1Gui.ButtonPressedMessage(2, x, y, z));
				WallChange1Gui.handleButtonAction(entity, 2, x, y, z);
			}
		}));
		this.addButton(new Button(this.guiLeft + 132, this.guiTop + 8, 30, 20, new StringTextComponent("D"), e -> {
			if (true) {
				FnafModMod.PACKET_HANDLER.sendToServer(new WallChange1Gui.ButtonPressedMessage(3, x, y, z));
				WallChange1Gui.handleButtonAction(entity, 3, x, y, z);
			}
		}));
	}
}
